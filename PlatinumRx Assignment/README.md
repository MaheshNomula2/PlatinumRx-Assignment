...
## Assumptions
1. All datetime columns store values in standard `YYYY-MM-DD HH:MM:SS` format.
2. "Most/Least ordered" in Q4 refers to total **quantity** of items ordered.
3. "Most valuable customers" in clinic Q2 means highest total **spend**.
4. Profit = Revenue − Expenses. A zero-expense month is not loss-making.
5. In Q5 (Hotel), `DENSE_RANK` is used so ties at rank 1 don't skip rank 2.
